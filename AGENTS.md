...
Taskfile.yml                       Build/test/deploy commands
...
- `config/crd/bases/*.yaml` - from `task manifests`
- `config/rbac/role.yaml` - from `task manifests`
- `config/webhook/manifests.yaml` - from `task manifests`
- `**/zz_generated.*.go` - from `task generate`
...
```
task manifests  # Regenerate CRDs/RBAC from markers
...
```bash
task test              # Run unit tests (uses envtest: real K8s API + etcd)
task run               # Run locally (uses current kubeconfig context)
```
...
task container-build container-push IMG=$IMG  # Or: kind load container-image $IMG --name <cluster>
task deploy IMG=$IMG
...
task build-installer IMG=<registry>/<project>:tag
```
...
**For development:**
```bash
task helm-deploy IMG=<registry>/<project>:<tag>          # Deploy manager via Helm
task helm-deploy IMG=$IMG HELM_EXTRA_ARGS="--set ..."    # Deploy with custom values
task helm-status                                         # Show release status
task helm-uninstall                                      # Remove release
task helm-history                                        # View release history
task helm-rollback                                       # Rollback to previous version
```
...
task container-build container-push IMG=$IMG
```
